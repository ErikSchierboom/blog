...
```js
var target = ko.observableArray().extend({ paged: { pageNumber: 2, pageSize: 5 } });
...
```js
var target = ko.observableArray([1,2,3,4,5]).extend({ paged: { pageSize: 2 } });
...
But what if someone wants to supply their own page generator? Ideally, you should be able to just *plugin* your own page generator (relevant: [open/closed principle](http://en.wikipedia.org/wiki/Open/closed_principle)). The first step towards realizing this is defining what the signature of a page generator is. Let's define it as a function that takes a paged observable and returns an array of pages.
...
```js
var custom = ko.observableArray([]).extend({ paged: { pageGenerator: 'custom' } });
